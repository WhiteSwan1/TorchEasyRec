...
class Tiger(BaseModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        name_to_feature_group = {
            x.group_name: x for x in model_config.feature_groups
        }
        self._history_group_name = cfg.history_group_name
        self._history_feature_name = name_to_feature_group[
            cfg.history_group_name
        ].feature_names[0]
        if self._num_user_bins > 0:
            self._user_group_name = cfg.user_group_name
            self._user_feature_name = name_to_feature_group[
                cfg.user_group_name
            ].feature_names[0]
        else:
    ...
    def _extract_history_sids_and_mask(
        ...
        """Densify history_sids KJT into (B, L_tokens) tensor + 0/1 mask."""
        jt = batch.sparse_features[self._history_group_name].to_dict()[
            self._history_feature_name
        ]
        values = jt.values()
        lengths = jt.lengths()
    ...
    def _extract_user_id(self, batch: Batch) -> Optional[torch.Tensor]:
        """Read user_id from the configured user feature group (if enabled)."""
        if self.user_embedding is None:
            return None
        jt = batch.sparse_features[self._user_group_name].to_dict()[
            self._user_feature_name
        ]
