...
GEMM 已经跑在约 90% 峰值（见第 5 节），所以唯一的抓手是**减少 FLOPs**，
而不是提高效率。FLOPs 与 token 数成正比，其中绝大部分在 MLP：

| 组成                            | 占线性层 FLOPs 比例 |
| ------------------------------- | ------------------: |
| MLP（gate/up/down，896x4864x3） |           **87.7%** |
| attention 投影（QKV + O）       |               12.3% |

MLP 宽度由 Qwen2.5-0.5B 这个 backbone 固定，所以实际可动的是 **token 数**和**数值精度**。

### 6.1 prompt 长度——实测 -15.5%

静态 prompt 是 71 个 token（历史槽位前 58 个、后 13 个），在 184,200 步训练的
每一步、每一行里都完全相同。

|                 |  平均每行 |          单步 |       峰值 |
| --------------- | --------: | ------------: | ---------: |
| 当前 JHK prompt | 422.9 tok |     233.38 ms | 20,147 MiB |
| 去掉 prompt     | 351.9 tok | **197.19 ms** | 18,051 MiB |
| 变化            |    -16.8% |    **-15.5%** |     -10.4% |

单步耗时几乎与 token 数等比例变化（token -16.8%，耗时 -15.5%），这正是 GEMM 主导的
另一种说法。这只是改一个配置字符串，收益约为 FA3 的 6 倍。但它改变了任务本身，
落地前必须做 HR 验证；把指令压缩到 10 个 token 左右，可以在语义改动更小的前提下
拿到大部分收益。

### 6.2 batch 大小——实测吞吐最多 +13.2%

batch 20 时 FA2 只用了 96 GB 中的 20 GB，还剩 76 GB。
...
收益到 batch 40-60 基本吃满。注意这**不降低单 token 的 GEMM 耗时**，
而是摊薄了与长度无关的优化器开销并改善了 GEMM 形状。它还会改变全局 batch
（20 x 32 卡 = 640），所以要么接受这个变化，要么保持全局 batch 不变、减少卡数——
batch 60 用 11 张卡大约也是 640，那是省成本而不是提速度。

### 6.3 FP8——唯一能把 GEMM 砍半的手段

H20 的 FP8 张量核约 296 TFLOP/s，是 BF16 的 2 倍。GEMM 占单步 60% 且已经跑到
BF16 峰值的 90%，因此 FP8 是理论上最大的抓手：最多能把单步砍掉约 30%。
它同时也是工作量最大的——需要接入 torchao `float8` 或 TransformerEngine，
还要认真做数值验证：毕竟当初选 FP32 master 权重，就是为了避免 Adam 小更新量
在 bf16 下的 ULP 下溢。

### 6.4 共享前缀计算——约 13%，且保留 prompt

历史槽位之前的 58 个 token，在每一行里产生的 hidden state 是完全相同的，
因为因果注意力让它们看不到后面的历史。现在这部分是每行各算一遍。
改成每个 batch 只算一次、各行共用这份 KV，可以省掉 19 x 58 = 1,102 个 token，
约占 8,460 的 13%。量级与裁剪 prompt 相当，但保留了 prompt 语义；
代价是要在 packed varlen 布局里把它实现出来。只有当 HR 验证说明 prompt 必须保留时，
才值得做。

### 6.5 没用的方向

- **更快的 attention kernel。** 只占单步 7%，FA2 已经吃掉了。
- **`torch.compile`。** 它作用于 cast/copy 和 elementwise 两桶，不减少 GEMM FLOPs。
- **梯度重算（gradient checkpointing）。** 它是用重算换显存，反而**增加** GEMM 耗时。
  还剩 76 GB 显存，没有理由付这个代价。
...
1. **`cast/copy` 占 14.1%，是 attention 的两倍。** 这是 `lm_parameter_dtype: FP32`
   ...
   是目前单项收益最大的方向。**
1. **`elementwise` 占 10.4%。** RoPE、残差相加、mask。适合做算子融合或对
   decoder block 上 `torch.compile`。
1. **优化器在 743 时占 6.3%，2048 时只占 2.6%。** 它与长度无关（各长度都是
   13.4 ms CUDA 时间），因为它只随参数量变化，不随 token 数变化。
1. **attention 占 7%。** FA2 已经把这块收走了，FA3 再加 2.5%。
...
- **GEMM 本身见第 6 节。** prompt 长度这一项实测 -15.5%，除 HR 验证外没有成本；
  FP8 是唯一能把它砍半的手段。
